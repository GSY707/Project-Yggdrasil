...
def test_boot_behavior_constitution_is_stable_and_scene_specific_text_stays_outside_boot() -> None:
    ...
    assert "优先进入已有合适工作节点" in compiled.boot_sections["behavior_constitution"]
    assert "先修改节点目标/问题/证据要求" in compiled.boot_sections["behavior_constitution"]
    assert "只有没有合适节点时才新建子节点或叶子节点" in compiled.boot_sections["behavior_constitution"]
...
def test_compile_runtime_prompt_prefers_formal_memory_tools_over_memory_write_tags() -> None:
    ...
    assert "优先进入已有节点，其次修改已有节点" in response_requirements
    assert "最后才新建节点" in response_requirements
    ...
    assert "<work-node-update nodeId=\"...\"" in response_requirements
    ...
    assert "每个 LLM window 最多输出一个会改变工作树状态的 directive" in response_requirements
    ...
    assert "不能作为已经进入、修改或完成 leaf 的证据" in response_requirements
