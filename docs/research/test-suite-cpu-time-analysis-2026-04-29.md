...
## 9. 阶段 2 与阶段 3 落地后补充测量

2026-04-30 已继续完成阶段 2 和阶段 3，并在本机安装真实 Redis：

- `app_catalog.py` 增加应用目录进程级 cache。
- `prompting.py` 增加 prompt registry cache，并允许 `compile_runtime_prompt()` 直接复用已解析 registry 和 tool descriptors。
- `tool_runtime.py` 增加 tool descriptor cache。
- `mcp_bridge.py` 改为显式控制路径刷新 snapshot，binding miss 不再在热路径里触发全量 sync。
- builtin MCP server 改为 keep-alive。
- 本机通过 `winget install Redis.Redis` 安装 Redis，并验证 `127.0.0.1:6379` 返回 `PONG`。

针对阶段 2/3 的关键热项，补做了一轮最小测量：

| 项目 | 之前 | 优化后 | 说明 |
| --- | ---: | ---: | --- |
| `build_application_catalog_snapshot()` | 22.76 ms | 0.20 ms | 之前通过每次清空 cache 模拟无缓存行为 |
| `assemble_prompt_registry()` | 69.78 ms | 0.75 ms | 之前通过每次清空 cache 模拟重复 hook 收集 |
| `resolve_registered_tool_descriptors()` | 45.09 ms | 0.70 ms | 之前通过每次清空 cache 模拟重复插件加载 |
| `compile_runtime_prompt()` | 78.53 ms | 0.04 ms | 优化后显式复用 registry 与 tool descriptors |
| `mcp.read.read_file` | 715.41 ms | 2.31 ms | 之前关闭 `workspace-read` keepAlive，优化后使用 keep-alive |
| Redis `flushdb()` 实机 | 2046.80 ms | 2.81 ms | 之前为不可达 Redis 历史基线，优化后为真实本机 Redis |

补充结论：

1. 阶段 2 说明当前 runtime 的大量开销确实来自重复装配，而不是 prompt 字符串拼接本身。
2. 阶段 3 说明 builtin MCP 的主要问题确实是子进程启停，而不是 Python 函数本身。
3. 当 Redis、prompt registry、tool descriptors 和 builtin MCP 都进入 warm path 后，本地路径已经开始接近“远小于 LLM 时延”的目标形态。

## 10. 阶段 4 落地后补充测量

2026-04-30 已完成阶段 4 的第一轮收敛：

- `llm_runtime.py` 增加 `strict`、`default`、`lean` 三档审计级别。
- `strict` 保留阶段 4 之前的全量同步写盘语义。
- `default` 改为在 request/response/compiled prompt 工件中写入关键元数据、message digests、tool/round 摘要和 timings，不再默认同步落完整 transcript 与 `rawResponse`。
- `lean` 进一步把 request/response 工件压缩为更小的摘要形态，适合本地 benchmark 与开发联调。

针对这三档模式，在与 pytest 基本一致的本地 SQLite + memory coordination 环境下，补做了一轮最小测量：

| 项目 | strict | default | lean |
| --- | ---: | ---: | ---: |
| `writeInitialRequestMs` | 0.49 ms | 0.48 ms | 0.48 ms |
| `rewriteRequestTranscriptMs` | 0.53 ms | 0.49 ms | 0.54 ms |
| `writeResponseMs` | 0.43 ms | 0.37 ms | 0.33 ms |
| request 工件体积 | 21081 B | 11435 B | 11324 B |
| response 工件体积 | 1970.8 B | 1021.2 B | 891.4 B |
| compiled prompt 工件体积 | 13532 B | 9309 B | 9281 B |

补充结论：

1. 阶段 4 的主要收益已经从“减少无意义等待”转成“收缩默认同步写盘 payload”。因此时间改善是小幅、稳定的，体积改善更显著。
2. `strict -> default` 已把 request 工件压缩约 45.76%，response 工件压缩约 48.19%，compiled prompt 工件压缩约 31.21%。
3. 由于测量使用的是 fallback 样本，response 本身并不大，所以 `writeResponseMs` 只从 `0.43 ms` 降到 `0.37 ms`。真实 live LLM 或多轮 tool transcript 下，这类体积收缩会更有意义。
4. `lean` 适合作为本地 benchmark 或调试会话的进一步瘦身档位，但常规开发默认保留 `default` 更稳妥，因为它仍然保留了摘要级别的审计可读性。