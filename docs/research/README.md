...
- `世界树计划正式项目定义.md` - 当前最高层项目定义入口：以 LLM 为核心，把代码定位为服务 LLM 的世界环境，并给出正式定义组成、核心边界和 E0 入口。
- `project-definition/` - 正式项目定义支撑文档：承接哲学与心理学基础、运行语义、阶段验收和特性审查规则。
- `新增特性，工作流.md` - 新工作流草稿：空间深度切片、按需能力挂载、软中断、多 Agent 协作、语义冲突仲裁和拉取式消息队列。

当前重新定义项目时，应先读正式项目定义，再把其他研究文档作为来源、证据或候选特性处理。
