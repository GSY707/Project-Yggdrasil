...
CLI 与服务入口会自动加载仓库根 `.env`。`YGGDRASIL_STATE_ROOT` 应配置为状态根目录，例如 `.yggdrasil`；`resolve_state_dir()` 会在其下自动使用 `state/` 子目录。
LANGFUSE_SECRET_KEY=
...
| 完整 Docker Compose 产品栈 | 预览可验证 | `corepack pnpm product:up` | 使用 `infra/docker-compose.product.yml`；`infra/product.env` 会优先于模板；已提供备份、恢复、快照列表、升级、回滚和 provider key 启动阻塞，正式发行前仍需多版本冷启动/升级演练。 |
| 桌面封装 | 未签名安装包预览 | `packaging/desktop/windows/Yggdrasil Installer.cmd` / `Yggdrasil Tray.cmd` | Windows 未签名安装/卸载、托盘控制器、启动/停止/状态/日志/备份/恢复/快照/升级/回滚、更新检查和手动应用入口已提供；签名和静默自动更新未完成。 |
| 托管 / SaaS | 计划中 | 尚未发布 | 已进入产品路线图；当前不能承诺官方托管、商业支持或 uptime。 |
| 官方远端数据服务 | 计划中 | 尚未发布 | 远端数据服务契约草案见 `docs/specs/remote-data-service-contract-v0.1.md`；实现仍需账号、租户、加密、保留和审计策略。 |
...
`infra/docker-compose.yml` 仍只代表开发依赖栈；不要再把它写成完整产品发行物。产品栈的数据卷边界见 `infra/docker-compose.product.yml`、`infra/product.env.template` 和未跟踪的 `infra/product.env`。真实 provider key 不得写入模板、镜像、测试 fixture 或文档。
...
| `tests/api/test_data_governance_api.py` | 数据治理 manifest、备份路由、删除 dry-run 审计、运行中任务阻塞、删除前备份与 task 级硬删除证明 |
| `tests/api/test_provider_configuration_api.py` | provider key 配置状态、`/health.providerStatus` 和密钥值不泄露 |
