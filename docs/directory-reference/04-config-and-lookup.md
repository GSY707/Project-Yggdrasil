...
| `docs/development/LLM_LIVE_WORKFLOW_AND_WORK_TREE_RERUN_AUDIT_2026_06_28.md` | LLM live 工作流程与工作树复跑审计：记录储能 real-task 重跑、完成后追问、每步反思、批评 revision 继续三组实验，固化 root-only 惯性、observed tool call 证据、cache usage 和行为记录器修复 |
| `docs/development/LLM_WORK_TREE_HARD_PROMPT_EXPERIMENTS_2026_06_29.md` | LLM 工作树硬提示实验记录：记录工具末尾强提醒、工具调用即 leaf 示例、leaf 自言自语示例、DeepSeek V4 Pro、leaf 执行/父节点评估重跑、DeepSeek 批评后继续，以及 auto-unfinished + 每节点 5 次 toolcall 软预算；区分 revision 继续、工作树使用行为与交付质量 |
