...
4. 工作树是上下文卫生工具；短小、单步、上下文干净的任务可以直接完成，需要隔离噪声、重复项、候选方向或局部实验时再创建/进入工作节点。
...
1. 简单任务直接完成；只有在需要隔离噪声、并行方面、重复项、候选方向或局部实验时，才创建或进入工作节点。
2. child 完成或失败后，应把父节点真正需要的结论、证据、已废弃路线、风险和下一步建议写成摘要，避免完整过程回灌。
