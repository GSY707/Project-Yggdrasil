...
- 取代范围：v0.2 为当前运行时协议；工作树使用策略以 2026-06-28 的上下文卫生口径为准。
- 设计来源：
...
1. LLM 在初次苏醒与后续起始状态中都知道自己的物理接口在哪里。
2. LLM 在进入任务前都能通过根指针找到身份、世界、能力/工具/知识索引和系统宪法。
3. 当前程序计数器属于任务级工作状态；只有在读取工作状态后，LLM 才恢复当前工作树节点。
4. 工作树是上下文卫生工具；root/非叶子节点负责高层视角、流程控制、方向重估和信息合并，叶子节点负责具体执行，执行产生过程噪声、重复项、候选方向或局部实验时创建/进入工作节点。
5. LLM 可以自然下潜、总结上浮、委派子任务、写入记忆并处理冲突；runtime 只提供边界、审计、恢复和提示，不替 LLM 做普通语义决策。
6. 安全、预算、不可逆操作、对外发布和用户审批仍是硬边界；普通任务完成路径由应用配置和任务现场决定。
...
```text
编写 -> 编译 -> 初次苏醒 -> 形成起始状态 -> 待机 -> 读取工作状态 -> 运行 -> 等待批准结束 -> 结束
```
...
- `[ID: 003 我要干什么]` 下的通用工作协议、工作状态读取入口和待机入口。
- `[NODE_ID: SYS_ROOT_PROTOCOL]` 下的系统宪法和底层协议。
- 能力索引、工具索引、工作索引、知识索引。

### 2.3 初次苏醒

初次苏醒是 LLM 第一次从无状态模型变成世界树通用 Agent 的过程。它只做唤醒、寻址、最低行为宪法和起始状态形成，不读取任何具体工作状态，也不做业务规划。
...
5. 如果有输入，先读取任务级工作状态，再创建或恢复工作树节点并进入运行。
...
1. root/非叶子节点应先确认任务目标、当前节点职责和是否需要拆分；具体执行应尽量放到 child/leaf，避免 root 堆积过程噪声。
2. child 完成或失败后，应把父节点真正需要的有用信息、证据/文件/记忆引用、已废弃路线和风险带回，避免完整过程回灌；下一步由父节点重新审视后决定。
3. `dependsOn` 是硬依赖；`relationIds`、ready-set、runtime hints 和 delivery readiness 是信息线索。
4. runtime 可插入预算、树深和安全警戒，但这些警戒只提供边界信息，不替代 LLM 的现场判断。
...
Boot Prompt 的目标不是教 LLM 做具体任务，而是完成系统级唤醒与起始状态形成。它类似 init 进程，只负责加载驱动、挂载文件系统、设置最低权限，并在任务开始前交出“如何读取工作状态”的入口，而不是直接交出当前任务节点。
...
5. 能力、工具、工作、知识都通过索引地图寻址；其中工具和知识在初次苏醒阶段默认只加载索引，不加载全文。
...
2. 禁止一次性打包大量信息强行写入。
3. 长文本、大量未知文件、仓库扫描、资料预读只有在需要并行、隔离上下文或独立工作流时才委派 Sub-Agent。
4. 长期有价值的新知、约束、失败原因和关键关联应尽早写入记忆或正式工件。
5. 节点命名必须以 `questions_it_answers` 作为路牌。
6. 只能信任同权限或更高权限记忆；低权限记忆只能作为待验证输入。
7. 遇到写冲突必须走版本重试、追加日志或提案合并，不得静默覆盖。
8. 需要父节点合并、后续复用或跨窗口恢复时，应写入工作节点摘要。
9. child 完成或失败后只带回父节点需要的摘要；下一步由 LLM 结合任务现场、工具证据、用户要求和硬依赖判断，runtime 不直接替代普通语义决策。

### 3.6 pcRecovery

必须包含现场恢复；但这里的现场恢复分为两层：初次苏醒默认只恢复到起始状态，任务级恢复才允许注入当前工作节点。
...
1. 初次苏醒默认 `currentWorkingNodeId` 为空；只有任务级工作状态读取完成后，才允许注入当前工作节点。
2. 如果 `currentWorkingNodeId` 非空，必须出现 `<Working_Node: {id}>`。
3. `pcMemo` 是上一窗口给当前窗口的短备忘录，不得替代工作树节点摘要。
4. `resumeMessage` 和 `restartMessage` 只能出现一次。
5. 恢复态不得重新从初始计划开始，除非工作树损坏且无法修复。
...
1. 你的能力：加载能力索引与能力入口，说明 LLM 可以读写什么、如何整理记忆、如何委派。
2. 你的工具：加载工具索引与工具入口，说明当前可调用工具和 MCP 能力；初次苏醒不加载全部工具全文。
3. 你的工作：只加载通用工作协议、待机状态和工作状态读取入口；具体任务工作树与当前节点属于任务级读取结果。
4. 你的知识：优先加载知识索引与知识入口，初次苏醒不加载全量知识正文。

如果预算不足：

1. 能力、工具和知识都只加载索引，不加载全文。
2. 工作只保留待机入口与工作状态读取入口，不预加载任务级 current node。
3. 进入任务后，知识按当前节点检索，不加载全局知识。

### 4.3 根挂载规则

1. Kernel 必须挂载根指针；当前工作状态和当前工作节点只在任务级启动后挂载。
...
3. `runtimeSections` 是任务级当前窗口工作集；只有在工作状态读取完成后才允许注入 `currentWorkTree` 与 `currentWorkingNode`。
