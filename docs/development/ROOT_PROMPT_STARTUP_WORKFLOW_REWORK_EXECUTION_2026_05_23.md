...
本轮变更的核心结论是：世界树运行时应从“代码强控 LLM 的任务状态机”转为“代码为 LLM 提供记忆、身体、边界、协作和反馈世界”。Boot Prompt 只负责唤醒、寻址和最低生存法则；工作树负责执行栈和工作记忆；工作流程由 LLM 基于当前工作节点、父节点编排位置、记忆树和工具索引主动推进。

## 1. 执行原则

1. 记忆树是长期状态主体，上下文窗口以当前工作缓存为主，但允许保留有限线性 continuation 轨迹以提升 prefix cache 命中和续跑稳定性。
2. Prompt 不承载业务知识，只提供隐性倾向、启动法则和当前现场恢复信息。
3. 工作树不是外部项目管理清单，而是 `我要干什么` 根分支下的动态工作记忆。
4. 系统维护指针、版本、权限、审计和工具边界；父节点中的 LLM 负责语义判断、节点摘要、经验上浮、拆 leaf 与续跑决策。
...
8. 工作树必须配套 `WorkContextStack`：下探子节点时 push frame，完成或失败子节点时 pop frame 回编排父级上下文并追加子节点摘要；是否进入 sibling、继续拆 leaf 或请求外部输入，由父节点决定。
...
回到 `分过程1` 后，下一步不是由 runtime 自动决定，而是由 `分过程1` 这个编排父节点决定：继续最细节执行2、继续拆 leaf、直接上浮或请求外部输入。分过程1完成后再 pop 回执行节点1，由执行节点1继续编排分过程2。这个流程依靠 `WorkContextStack` 维护上下文窗口和模型前缀缓存；窗口重启只作为预算、安全停止或 provider 限制下的 fallback。
...
5. Prompt 不再要求模型输出格式化待办，工作树父子结构承担计划清单；恢复态 prompt 应保留父节点编排所需的有限线性轨迹，而不是只剩静态树快照。
...
5. 子节点完成后默认 pop 回父帧，不默认触发窗口重启，也不由 runtime 直接替父节点跳到下一个 sibling。
...
8. 父节点需要显式决定是否进入已有 child、继续创建 leaf 或等待外部输入；代码只提供预算、树深和安全警戒。

验收：

- 单个任务可以经历下潜、执行、摘要、上浮。
- 子节点完成后能回到父级上下文，并由父节点决定是否继续下一个兄弟节点或继续拆 leaf。
- 未写摘要不能完成或跳出当前节点。
- 任务最终需要显式批准完成。
