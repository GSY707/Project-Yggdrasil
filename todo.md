...
- 本轮已完成一项副线治理：`tests/test_m9_modules.py` 已拆分为 5 个 M9 专项测试文件，并通过定向 pytest 验证。

## 顶层路线
- 路线文档：`docs/research/final-goal-roadmap-2026-04-30.md`
- 当前执行锚点：先完成 Gate 1，也就是“在窄路径里证明用户给目标后系统可以交付”。
- 在 Gate 1 闭合前，不重开大规模功能扩展。
- 提示词当前目标：先把能力稳定到 `P1`，并为 `P2` 的结构化编译链和基础 few-shot 补齐缺口。

## 当前阶段完成度
- 按 Gate 1 出口标准计：**0 / 4 项闭合**，当前阶段目标尚未完成。
- 按执行准备度估算：约 **35%**；已完成沙箱入口、验证材料冻结、看板/README/开发文档同步，以及一轮低风险大文件拆分示范。
- 当前主要缺口集中在：专用沙箱内部试跑证据、Core API HTTP 实测延迟、首 token / 首次有效输出观测。
...
3. 为首轮试跑补齐评分表、录屏、trace 与工件目录，形成证据闭环。
4. 将前 5 个阻塞因素按“配置 / 速度 / 体验 / 能力”分类沉淀。
5. 实测 Core API HTTP 关键路径的 P50 / P95。
6. 将 HTTP 实测值回写到 `docs/QUALITY_BASELINE.md`。
7. 补首 token / 首次有效输出级别的首响观测。
8. 在内部试跑中验证 safe-stop / resume 的连续性与恢复成功率。
9. 继续拆分下一个大文件，优先 `repositories.py` 或 `services.py`。
10. 为大文件拆分建立固定回归入口，避免副线治理长期回潮。
