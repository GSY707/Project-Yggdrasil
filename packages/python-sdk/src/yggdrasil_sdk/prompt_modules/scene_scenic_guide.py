...
class ScenicGuideSceneModule(BaseModulePlugin):
    module_id = "scene-scenic-guide"
    ...
    def enable_preflight(self, payload: dict[str, object]) -> dict[str, object]:
        return {"status": "ok", "summary": "Scenic guide scene module preflight passed."}

    def report_health(self, payload: dict[str, object]) -> dict[str, object]:
        return {"status": "healthy", "summary": "Scenic guide scene module is ready."}

    def register_seed_templates(self, payload: dict[str, object]) -> dict[str, object]:
        return {
            "seedTemplates": [load_module_prompt_asset(self.module_id, "scenes/scenic-guide.yaml")],
            "moduleId": self.module_id,
        }


plugin = ScenicGuideSceneModule()