...
class WritingEpicSceneModule(BaseModulePlugin):
    module_id = "scene-writing-epic"
    ...
    def enable_preflight(self, payload: dict[str, object]) -> dict[str, object]:
        return {"status": "ok", "summary": "Writing epic scene module preflight passed."}

    def report_health(self, payload: dict[str, object]) -> dict[str, object]:
        return {"status": "healthy", "summary": "Writing epic scene module is ready."}

    def register_seed_templates(self, payload: dict[str, object]) -> dict[str, object]:
        return {
            "seedTemplates": [load_module_prompt_asset(self.module_id, "scenes/writing-epic.yaml")],
            "moduleId": self.module_id,
        }


plugin = WritingEpicSceneModule()