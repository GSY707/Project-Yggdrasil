...
def _has_formal_delivery_sections(text: str) -> bool:
    ...
    return any(all(section in candidate for section in headings) for candidate, headings in ((normalized_lower, required_heading_sets[0]), (normalized, required_heading_sets[1])))
def _work_tree_all_leaves_complete(work_tree: Any) -> bool:
    """Return True when every non-root work-tree node has reached a terminal status.

    Used as a delivery-gate relaxation: when all leaf nodes are completed the root
    synthesis window is allowed to proceed to awaiting-approval even if the model
    did not produce the four formal section headings.
    """
    ...
    leaf_nodes = [n for n in nodes if str(getattr(n, "id", "") or "") != root_node_id]
    if not leaf_nodes:
        return False
    return all(str(getattr(n, "status", "") or "") in terminal for n in leaf_nodes)
...
def _window_restart_trigger(request: dict[str, Any], runtime_metrics: dict[str, Any], effective_context: list[dict[str, Any]]) -> tuple[str | None, int]:
    # Legacy helper kept for compatibility with historical artifacts/tests.
    # Hot path no longer performs automatic window restart transitions.
